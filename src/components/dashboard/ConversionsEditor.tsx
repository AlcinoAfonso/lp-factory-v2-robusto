...
// ✅ TIPOS SIMPLIFICADOS - sem dependências externas
...
export function ConversionsEditor({ clientId, lpId, lpData }: ConversionsEditorProps) {
  ...
  const detectConversionsFromLP = useCallback((lpData: any): EditableConversion[] => {
    const detected: EditableConversion[] = [];
    
    // Mock detections baseado em padrões comuns
  ...
  const loadConversions = useCallback(async () => {
    ...
    try {
      ...
      detectedConversions.forEach(conv => {
      ...
      console.info(`✅ ConversionsEditor: ${detectedConversions.length} conversões carregadas`);
    } catch (error) {
  ...
  // ✅ SALVAR CONVERSÃO INDIVIDUAL - API CORRIGIDA
  const saveConversion = async (conversionId: string) => {
    ...
    try {
      // ✅ CORREÇÃO: Usar API save-and-deploy correta
      const trackingData = {
        client: clientId,
      ...
      // ✅ CORREÇÃO: Chamar API save-and-deploy
      ...
      setGlobalMessage({ 
        type: 'success', 
        text: `✅ Conversão "${conversion.label}" salva e publicada! Deploy automático iniciado.` 
      });
  ...
  // ✅ SALVAR TODAS AS CONVERSÕES - API CORRIGIDA
